...
epochsize <- habitats * stages
S <- matrix(rep(0,epochsize*epochsize), nrow=epochsize)

# make the R matrix
R <- matrix(rep(0,epochsize*epochsize), nrow=epochsize)
smallMatrix <- matrix(c(seedMigrationRate,0,0,0), nrow = 2, byrow = TRUE)
for(i in 0:(habitats-1)){
  # to the left
  hab <- (i-1) %% (habitats)
  for (x in 0:1){
    for(y in 0:1){
      R[((i+1)*2)-1+x,((hab+1)*2)-1+y] <- smallMatrix[x+1,y+1]
    }
  }
  # to the right
  hab <- (i+1) %% (habitats)
...
# make the M matrix
M <- matrix(rep(0,epochsize*epochsize), nrow=epochsize)
smallMatrix <- matrix(c(0,0,0,pollenMigrationRate), nrow = 2, byrow = TRUE)
for(i in 0:(habitats-1)){
  # to the left
  hab <- (i-1) %% (habitats)
  for (x in 0:1){
    for(y in 0:1){
      M[((i+1)*2)-1+x,((hab+1)*2)-1+y] <- smallMatrix[x+1,y+1]
    }
  }
  # to the right
  hab <- (i+1) %% (habitats)
